...
excel_file = "customer_data.xlsx"  # Path to your Excel file
df = pd.read_excel(excel_file)  # Read Excel file into DataFrame

# Step 2: Connect to SQLite (or create the database)
conn = sqlite3.connect("customer_database.db")  # Create database file
cursor = conn.cursor()
...
query_results.to_csv("query_results.xlsx", index=False)  # Save to CSV
